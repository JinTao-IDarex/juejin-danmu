Create a single horizontal sprite strip for the Codex app digital pet `cyber-ronin` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: 赛博朋克武士浪人：霓虹紫发光武士刀、青色发光眼罩护目镜、机械义体右臂、深色和服束带、传统浪人发型（顶髻+鬓发）.
